...
2. 对内核有更改或权限需求的应用需慎重容器化。对主机内核的修改也将同时影响主机之上的其它容器。

3. 虽然原则建议每容器运行一个进程，但通常多进程容器在一些场景不可避免，请在充分理解应用的基础上做应用服务拆分。

4. 结合平台日志方案，尽量将容器日志集中存放在一个volume from 的容器内，或支持syslog

5. 不建议容器化需要极致的网络性能要求的应用\(docker 网络有一定性能损耗）
