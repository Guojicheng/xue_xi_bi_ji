...
采用rancher catalog 商店的mongodb replica 集群， 在此基础上添加需要创建的用户及密码，语法如下：
...
```
FROM redhat/6.4MAINTAINER wise2c
##准备依赖环境
#RUN yum -y install gcc make gcc-c++ openssl-devel wget
## 修改容器时钟信息
RUN rm -rf /etc/localtime && ln -s /usr/share/zoneinfo/Asia/Shanghai /etc/localtime
#ADD clock /etc/sysconfig/clock \
### 安装PYTHONADD setuptools-0.6c11-py2.6.egg /RUN /bin/sh /setuptools-0.6c11-py2.6.egg
##准备NODE 安装包
ADD node-v0.12.2-linux-x64.tar.gz /ykt/
RUN cd /ykt \
  && mv node-v0.12.2-linux-x64 node
WORKDIR /ykt/node/bin/
ENV NODE_HOME=/ykt/nodeENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/root/bin:/bin"
### 加云课堂应用
ADD yunketang.tar /ykt/RUN 
chmod 777 /ykt/yunketang/*.sh  \
  && pm2 start redhat
### 加端口宣告到主机EXPOSE 7100
CMD pm2
```
...
本例中mongodb 集群及redis 集群理论上只为云课堂应用服务，不需暴露对外服务端口，故都不做对宿主要的端口映射，只在容器内宣告服务端口号。云课堂服务启动时使用rancher 的服务link 连接至mongodb replica 及redis ，支持服务发现。在此基础上，设置修改配置文件的变量及传参如下：
