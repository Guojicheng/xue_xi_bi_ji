...
通过第三方应用提供商按照 标准化的需求调研表格提交相应内容，运维方与应用所有方有一个标准的沟通界面。在此基础上，双方进行初步的会议沟通，确认容器化的主要工作内容及相关技术需求、时间规划、责任划分问题。并就主要的技术点进行初步论证。
...
2. **应用结构及组件情况（需要原有应用的附拓扑图、原有部署文档、安装、配置手册 ）**
...
** 阶段任务：**
 ...
 容器云平台管理员依据初步讨论结果 （阶段一会议纪录），确定应用是否适用于容器平台，及初步评估工作周期及工作量。就应用容器最终部署的初步方案的形成共识，具体包括（服务发现、负载机制，应用服务解构等）。预估相关工作的困难度及预设实施周期。
