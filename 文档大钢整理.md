...
```
 那么，涉及到具体工作流程规划，我们通过梳理将具体的工作分为四个阶段，对每阶段我们分别对如下几点作出描述 ：
```
...
在达成如上目标的情况下，双方人员有效沟通，确定应用是否适用于容器平台，及初步评估工作周期及工作量。就应用容器最终部署的初步方案的形成共识，具体包括（服务发现、负载机制，应用服务解构等）。预估相关工作的困难度及预设实施周期。

**阶段任务3:**

在达成技术实现共识的基础上，确定具体执行人员及具体工作进度安排表

（注： 在本文中，通常将任务中涉及的文档划分为两大类

**一 工作流程文档**

主要用于在工作中相互沟通确认工作内容，权责 分工等。 类似调研表格

**二 阶段交付物文档**

主要用于在当前阶段完成时，总结阶段工作结论及成果。该交付物同时作为启动下一阶段的必要的文档输入。）

##### **阶段任务1**

##### ** 输入说明：**

本阶段以接受容器化指令，并经直管领导确认为输入，具体的输入可以为确认邮件或申请表格形式。
