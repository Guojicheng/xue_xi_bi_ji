...
#### 3.1   我们会要求需要迁移到Rancher 容器云平台的应用尽可能的将现有应用的解构（微服务化）。

#### 3.2   需要解构后的单个服务（容器）能够支持冗余或横向扩展，以最大程度的避免单点故障增强高可用性。

#### 3.3   需要合理编排、定义应用中各个服务单元（容器）的启动顺序，服务单元\(容器）之间的服务发现机制（Ranche 平台本身提供服务基于内部dns的发现机制，推荐采用\)。

#### 3.4   需要重新整理、引入配置脚本选项或配置管理工具。以支持在Rancher 平台一键启动应用时，自定义变更应用参数的需求。（例如一个简单的用户访问mysql的场景，我们传入用户名、密码及数据库名称参数，mysql 的服务会依据传入的参数生成相应的库及用户，同时客户端也采用些配置登录数据库）

#### 3.5   需要利用Rancher 平台特性，合理设计应用的存储、网络、负载均衡架构 。

#### 3.6   制作容器镜像

#### 3.7  实现应用商店的一键部署（编写 应用说明，docker-compose , rancher-compose文件\)

那么，涉及不到具体工作中，我们通过梳理工作流程，将具体的分作分为四个阶段，对每阶段我们总结三个相关的工作内容要素，以明确平台运维方与应用所有方的职责 ，指导整个应用容器化工作的工作量、权责划分及进度安排 。

第一阶段： 应用梳理、评估阶段。

本阶段任务及目的：

本阶段主要目的为在接收到容器化需求后，通过标准化的调研表格及双方技术人员有效沟通，快速的梳理、评估现有应用转为在Rancher 平台标准的容器化

应用所需要的工作量、确定应用结构调整初步方案，同时需要确定人员安排、明确项目时间周期规划并细分时间节点。

阶段任务1： 通过设置标准化的需求调研表格，以达成容器平台运维方与应用所有方有一个标准的沟通界面。

阶段任务2:   在达成如上目标的情况下，双方人员有效沟通，确定应用是否适用于容器平台，及初步评估工作周期及工作量。就应用容器最终部署的初步方案的形成共识，具体包括（服务发现、负载机制，应用服务解构等）。预估相关工作的困难度及预设实施周期。

阶段任务3:    在达成技术实现共识的基础上，确定具体执行人员及具体工作进度安排表
...
#####     一为工作流程文档     主要用于在工作中相互沟通确认工作内容，权责 分工等。 类似调研表格

#####    二为阶段交付物文档    主要用于在当前阶段完成时，总结阶段工作结论及成果。该交付物同时作为启动下一阶段的必要的文档输入。）
